<x-jet-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Cuando la autenticación de dos factores está habilitada, se le solicitará un token aleatorio seguro durante la autenticación. Puede recuperar este token de la aplicación para smart-phone Google Authenticator') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('La autenticación de dos factores ahora está habilitada. Escanee el siguiente código QR usando tu phone\'s authenticator.') }}
                    </p>
                </div>
            ...
            @else
                ...
                @else
                    <x-jet-confirms-password wire:then="showRecoveryCodes">
                        <x-jet-secondary-button class="mr-3">
                            {{ __('Observar codigos de recuperacion') }}
                        </x-jet-secondary-button>
                    </x-jet-confirms-password>
                @endif
